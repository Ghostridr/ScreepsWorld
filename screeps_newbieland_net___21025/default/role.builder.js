...
var roleBuilder = {
    ...
    run: function (creep) {
        ...
        if (creep.memory.building) {
            ...
            if (target) {
                ...
                if (creep.build(target) === ERR_NOT_IN_RANGE) {
                    ...
                    creep.moveTo(target, { visualizePathStyle: Paths.roles.builder.build });
                } else if (
                    typeof target.progress === 'number' &&
                    typeof target.progressTotal === 'number' &&
                    target.progressTotal > 0 &&
            ...
            if (src && !Threat.isDanger(src.pos, creep.room.name)) {
                ...
                if (creep.harvest(src) === ERR_NOT_IN_RANGE) {
                    Say.changed(creep, 'MOVE');
                    creep.moveTo(src, {
                        visualizePathStyle: Paths.roles.builder.harvest,
                    });
                } else {
        ...
        // Stuck detection for builders: invalidate path to current target when stuck
        ...
        if (mm.stuck >= 3) {
            const claimedId = Build.claimedSiteId(creep.room.name, creep.name);
            const site = claimedId && Game.getObjectById(claimedId);
            const goal = site || null;
            if (goal) {
