...
module.exports.loop = function () {
    for (const spawnName in Game.spawns) {
        ...
        for (const src of sources) {
                ...
                for (let dx = -1; dx <= 1; dx++) {
                    for (let dy = -1; dy <= 1; dy++) {
                        ...
                        if (res === OK) {
                            ...
                            Log.info(
                                G.info(
                                    `site at ${x},${y} near ${src.id}`,
                                    [
                                        'Assign a builder to construct it.',
                                        'Lay roads for miner/hauler paths.',
                                    ],
                                    {
                                        room: room.name,
                                        source: src.id,
                                        file: 'manager.container.js',
                                    },
                                    'CONT_SITE'
                                ),
                                room.name
            ...
            if (tryPlace(true)) return; // first pass: dry tiles
            if (tryPlace(false)) return; // fallback: allow swamps
        }
    }
};
