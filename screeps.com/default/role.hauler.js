...
module.exports = {
    run(creep) {
        ...
        if (job) {
            ...
            if (creep.store.getFreeCapacity() > 0) {
                ...
                if (wr === ERR_NOT_IN_RANGE) {
                    creep.moveTo(src, {
                        visualizePathStyle:
                            Paths.roles.hauler.withdraw || Paths.roles.harvester.withdraw,
                    });
                ...
                if (tr === ERR_NOT_IN_RANGE) {
                    creep.moveTo(dst, {
                        visualizePathStyle:
                            Paths.roles.hauler.deliver || Paths.roles.harvester.deliver,
                    });
                } else if (tr === OK) {
        ...
        // Stuck detection: if pos unchanged N ticks, invalidate current path key
        const posKey = Mapper.toPosKey(creep.pos);
        const mem =
            creep.memory._moveMon ||
            (creep.memory._moveMon = { lastPos: posKey, stuck: 0, lastGoal: null });
        if (mem.lastPos === posKey) mem.stuck = (mem.stuck || 0) + 1;
        else {
            mem.stuck = 0;
            mem.lastPos = posKey;
        }
        if (mem.stuck >= 3) {
            var goalId = null;
            if (creep.memory.jobId) {
                var _j = HaulQ.get(creep.room.name, creep.memory.jobId);
                goalId = _j && _j.dstId;
            }
            const goalObj = goalId && Game.getObjectById(goalId);
            if (goalObj) {
                const k = Mapper.reuseKey(Mapper.toPosKey(creep.pos), Mapper.toPosKey(goalObj.pos));
                if (!Memory.pathCache) Memory.pathCache = {};
                Cache.del(Memory.pathCache, k);
            }
        }

        // Guidance/logs sparingly (fallback if .every is missing)
        const logEvery =
