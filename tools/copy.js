#!/usr/bin/env node
// tools/copy.js
// Copy every src/**/*.js (excluding this tools folder) into each server's /default directory
...
function listJs(dir) {
    ...
    for (const ent of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (ent.isDirectory()) {
            ...
            // Exclude this copy.js file and any other tooling scripts in tools/ from being copied.
            ent.isFile() &&
            p.endsWith('.js') &&
...
// Copy srcFile to targetDir/<module>.js
function copyOne(targetDir, srcFile) {
    ...
    if (preview) {
        console.log(`[PREVIEW] Would copy ${srcFile} to ${out}`);
    } else {
...
function main() {
    ...
    if (!files.length) {
        ...
        console.warn('[copy] Only main.js detected in src/.');
    }
    const preview = process.argv.includes('--preview');
    if (preview) {
        for (const t of targets) {
            ...
            console.log(`Target: ${relTarget}`);
            console.log('[PREVIEW] Would copy:');
        ...
        console.log('Preview mode: No files were actually copied.');
        return;
    } else {
        for (const t of targets) {
            console.log(`\nTarget: ${t}`);
            for (const f of files) copyOne(t, f);
        }
    }
    console.log(
        '\n=================================\nCompleted copying to',
        targets.length,
        'target(s) successfully.'
    );

    // --- Watch for changes and copy automatically ---
    const chokidar = require('chokidar');
    const watcher = chokidar.watch(SRC, {
        ignored: /(^|[/\\])\..|tools/, // ignore dotfiles and tools/
        persistent: true,
        ignoreInitial: true,
    });
    if (!preview) {
        watcher.on('change', (changedPath) => {
            // Changed file; copy to all targets
            if (!changedPath.endsWith('.js')) return;
            const rel = path.relative(SRC, changedPath);
            if (rel.startsWith('tools/')) return;
            if (/tools\/(copy|clear)\.js$/.test(rel)) return;
            for (const t of targets) copyOne(t, changedPath);
            console.log(`[watch] Copied changed file: ${changedPath}`);
        });
        watcher.on('add', (addedPath) => {
            // New file added; copy to all targets
            if (!addedPath.endsWith('.js')) return;
            const rel = path.relative(SRC, addedPath);
            if (rel.startsWith('tools/')) return;
            if (/tools\/(copy|clear)\.js$/.test(rel)) return;
            for (const t of targets) copyOne(t, addedPath);
            console.log(`[watch] Copied new file: ${addedPath}`);
        });
        watcher.on('unlink', (removedPath) => {
            // Remove from all targets
            const mod = toModule(removedPath);
            for (const t of targets) {
                const out = path.join(t, `${mod}.js`);
                if (fs.existsSync(out)) {
                    fs.unlinkSync(out);
                    console.log(`[watch] Removed: ${out}`);
                }
            }
        });
